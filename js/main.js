...
function create(){
  ...
  pewpew = game.add.audio('pewpew', 0.3, false); //".3" is to muffle sound
  ...
  lasers.createMultiple(30, 'laser'); //'laser' the sprite img 'beam'
  ...
  //Spawn enemy every 2 sec
  game.time.events.loop(Phaser.Timer.SECOND * 2, spawnEnemy, this);
}

function update(){
  player.body.velocity.set(0); //stop the player movement in loop when noDown
  //Keyboard detection
